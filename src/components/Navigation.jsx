      ...
      <div className="backdrop-blur-md bg-white/85 dark:bg-gray-800/60 border-b dark:border-gray-900 sticky top-0 z-100 h-[60px]">
         <div className="navbar container mx-auto flex justify-between py-2">
            <div className="flex gap-4 md:gap-5">
               <LinkProgresBars
                  to="/"
                  className="flex items-center rounded-lg font-semibold text-xs px-3.5 gap-2 bg-linear-to-r from-slate-700 to-slate-800 relative group overflow-hidden"
               >
                  <div className="absolute right-0 w-0 h-full bg-linear-to-t from-slate-800 to-slate-900 group-hover:w-full duration-500"></div>
                  <svg
                  ...
                  <span className="hidden md:block text-white dark:text-slate-200 relative">Kembali</span>
               </LinkProgresBars>

               {props.loadP == false ? (
                  <>
                     <div className="">
                        <p className="font-semibold text-md">{props.suratP}</p>
                  ...
                  <div className="animate-pulse ml-2 md:ml-7">
                     ...
                     <div className="grid grid-cols-3 gap-2 mt-1">
                        <div className="h-2 w-7 bg-slate-200 rounded-sm col-span-1"></div>
                        <div className="h-2 w-7 bg-slate-200 rounded-sm col-span-1"></div>
                        <div className="h-2 w-7 bg-slate-200 rounded-sm col-span-1"></div>
                     </div>
                  </div>
               ...
               <ul className="flex gap-2 items-center">
                  ...
                  <div className="relative inline-block text-left">
                     ...
                     <button
                        ...
                        className="flex items-center gap-1 px-3 py-2 border dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 shadow-xs hover:bg-gray-100 dark:hover:bg-gray-700"
                     >
                        <span>Ayat</span>
                        <svg
                           className={`w-5 h-5 duration-500 transition-transform dark:fill-gray-200 ${open ? "rotate-180" : ""}`}
                           xmlns="http://www.w3.org/2000/svg"
                           viewBox="0 0 24 24"
                        >
                     ...
                     {open && (
                        <ul className="absolute mt-2 z-20 w-20 p-2 max-h-[300px] overflow-y-auto rounded-md border dark:border-gray-700 shadow-lg bg-white dark:bg-gray-800/90">
                           {props.singleSP?.ayat?.map((sp) => (
                              <li key={sp.nomor}>
                                 <a
                                    href={`/surah/${numbertosurah[sp.surah]}#surahke${sp.nomor}`}
