...
const DetailSurahView = ({
   ...
   const shareAyat = useMemo(() => dataDetails?.ayat?.find((item) => item.nomor === shareState.ayat), [dataDetails?.ayat, shareState.ayat]);
   ...
   const handleNativeShare = async () => {
      ...
      try {
         ...
         const file = new File([blob], `${dataDetails?.nama_latin || "ayat"}-${shareState.ayat}.png`, {
            type: "image/png",
         });
   ...
   return (
      <>
         ...
         {bookStats == true ? (
            <div>
               <button className="btn-scroll hidden" id="scrollNoww" ref={() => document.getElementById("scrollNoww").click()} onClick={() => handleClickScroll()}>
                  Lanjutkan Membaca
               </button>
            </div>
         ) : null}
         ...
         <div className="container mx-auto selection:bg-blue-200 min-h-[90vh]">
            <div className={`fixed bg-white border border-slate-400 z-9999 top-[35%] p-3 rounded-l-xl w-[170px] duration-500 ${sideMenu ? "right-0" : "right-[-170px]"}`}>
               <button
               ...
               <div className="relative max-w-xs">
                  ...
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-300 text-sm">🕌</span>
               </div>
               ...
               <div className="modal md:items-start md:pt-10 px-3 mt-10">
                  <div className="modal-box w-full max-w-4xl">
                     ...
                     <h2 className="text-lg font-bold">
                        Deskripsi Surat {dataDetails?.nama_latin} <b className="font-serif">{dataDetails?.nama}</b>
                     </h2>
                     <div className="py-4" dangerouslySetInnerHTML={{ __html: dataDetails?.deskripsi }}></div>
                  </div>
               </div>
                     ...
                     {Loading == false ? (
                        dataDetails?.ayat?.map((single) => (
                           ...
                           >
                              ...
                              <div className="text-xl flex justify-between md:block gap-1 px-3 top-2 w-full mb-3 md:w-fit md:mb-0 md:px-0 absolute left-0 border-b border-dashed border-slate-200 dark:border-gray-700 pb-2 md:border-0">
                                 <div className="flex md:block w-full gap-2">
                                    ...
                                    <div className="dropdown w-8 md:w-full dropdown-bottom md:mt-1">
                                       ...
                                       <ul
                                          tabIndex={0}
                                          className={` absolute left-0 mt-2 z-50 menu p-1 text-[15px] shadow-md border rounded-md w-[150px] bg-white dark:bg-slate-800/60 backdrop-blur-md dark:border-gray-700 overflow-hidden transition-all duration-300 ease-in-out origin-top space-y-1
                                               ${openMenu === single.nomor ? "max-h-[300px] opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-0 pointer-events-none"}
                                            `}
                                       >
                                          ...
                                          {/* <li>
                                             <button className="p-0 px-1 text-left w-full" onClick={() => openShareCard(single)}>
                                                Share Card Ayat
                                             </button>
                                          </li>*/}
                                          {/* <li>
                              ...
                              <div className="arab sm:px-3 lg:pl-2 w-full md:w-[90%] lg:w-[94%]" style={{ fontSize: font.arab + "px" }}>
                                 {single.ar}
                                 {"  "}
                              ...
                              <p className="w-full sm:px-3 text-left mt-2 text-[15px] lg:w-[94%] lg:mt-7 nunito lg:pr-2" style={{ fontSize: font.idn + "px" }}>
                                 {single.idn}
                              </p>
                           </div>
                        ))
                                       ...
                                       <div className="animate-pulse flex space-x-4">
                                          <div className="flex-1 py-1">
                                             ...
                                             <div className="grid grid-cols-3 gap-2 mt-9">
                                                ...
                                                <div className="h-3 bg-slate-200 rounded-sm col-span-3"></div>
                                             </div>
                                          </div>
                                       </div>
                                    </div>,
                     ...
                     >
                        {dataDetails?.ayat?.map((single) => (
                           <span
                              ...
                              onClick={() => setActiveMenu(activeMenu === single.nomor ? null : single.nomor)}
                           >
                              {/* Teks Ayat + Nomor */}
                              {single.ar}{" "}
            ...
            {dataDetails?.ayat?.map((single) => (
               <div key={single.nomor}>
                  <input type="checkbox" id={`tafsir-modal-${single.nomor}`} className="modal-toggle" />
                  <div className="modal md:items-start md:pt-10 px-3">
                     <div className="modal-box w-full max-w-7xl bg-white dark:bg-slate-800">
                        <label htmlFor={`tafsir-modal-${single.nomor}`} className="btn btn-sm btn-circle absolute right-2 top-2">
                           ✕
                        </label>
                        <h3 className="text-lg font-bold">
                           Tafsir {dataDetails.nama_latin} Ayat ke : {single.nomor}
                        </h3>
                        <p className="py-1 text-[16px] leading-[25px] text-justify">{dataTafsir?.data?.tafsir[parseInt(single?.nomor)]?.teks}</p>
                     </div>
                  </div>
             </div>
            ))}
            ...
            <div className="fixed left-[-9999px] top-0 pointer-events-none opacity-0">
               <div
                  ref={shareCardRef}
                  className="w-[1080px] min-h-[1350px] bg-linear-to-br from-slate-950 via-slate-900 to-emerald-950 text-white p-16 flex flex-col justify-between"
               >
                  <div>
                     <div className="flex items-center justify-between">
                        <div>
                           <p className="text-2xl tracking-[0.35em] uppercase text-emerald-200/80">Al Quran Digital</p>
                           <h2 className="mt-4 text-5xl font-bold">{dataDetails?.nama_latin || "Surah"}</h2>
                           <p className="mt-2 text-xl text-slate-200">
                              Ayat {shareState.ayat || "-"} {dataDetails?.arti ? `- ${dataDetails.arti}` : ""}
                           </p>
                        </div>
                        <div className="text-right">
                           <p className="text-sm uppercase tracking-[0.4em] text-emerald-200/80">Share Card</p>
                           <p className="mt-3 text-4xl font-arabic">{shareAyat ? toArabicNumber(shareAyat.nomor) : ""}</p>
                        </div>
                     </div>

                     <div className="mt-16 rounded-4xl border border-white/10 bg-white/8 backdrop-blur-md p-12 shadow-2xl">
                        <p className="text-right text-[54px] leading-[2.2] font-arabic text-white">{shareAyat?.ar || " "}</p>
                        <div className="mt-10 border-t border-white/10 pt-8">
                           <p className="text-2xl leading-[1.8] text-slate-100">{shareAyat?.idn || "Pilih ayat untuk membuat share card."}</p>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="flex items-end justify-between gap-8">
                     <div>
                        <p className="text-sm uppercase tracking-[0.35em] text-emerald-200/70">Dibagikan dari</p>
                        <p className="mt-3 text-2xl font-semibold">{window.location.origin}</p>
                     </div>
                     <div className="text-right">
                        <p className="text-sm text-slate-300">Bismillah</p>
                        <p className="text-lg text-emerald-200">{dataDetails?.tempat_turun || ""}</p>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className={`modal ${shareState.open ? "modal-open" : ""}`}>
               <div className="modal-box w-full max-w-5xl bg-white dark:bg-slate-900">
                  <button type="button" className="btn btn-sm btn-circle absolute right-2 top-2" onClick={closeShareModal}>
                     ✕
                  </button>

                  <h3 className="text-lg font-bold">Share Card Ayat {shareState.ayat || "-"}</h3>
                  <p className="text-sm text-slate-500 mt-1">{dataDetails?.nama_latin}</p>

                  <div className="mt-4 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-950">
                     {shareState.loading ? (
                        <div className="h-[420px] flex items-center justify-center text-white">Sedang menyiapkan kartu...</div>
                     ) : shareState.imageUrl ? (
                        <img src={shareState.imageUrl} alt="Share card ayat" className="w-full h-auto" />
                     ) : (
                        <div className="h-[420px] flex items-center justify-center text-white">Klik share pada menu ayat untuk membuat kartu.</div>
                     )}
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2 justify-end">
                     <button type="button" className="btn btn-outline" onClick={downloadShareCard} disabled={!shareState.imageUrl}>
                        Download PNG
                     </button>
                     <button type="button" className="btn btn-primary" onClick={handleNativeShare} disabled={!shareState.imageUrl}>
                        Share
                     </button>
                  </div>
               </div>
            ...
            <div className="flex justify-between w-max p-2 gap-2 border border-slate-200 shadow-md rounded-xl mb-3 fixed z-99 bottom-0 left-1/2 -translate-x-1/2 backdrop-blur-md bg-white/85 dark:bg-gray-800/50 dark:border-gray-700">
               {(() => {
                  if (dataDetails?.surat_sebelumnya !== false) {
                     return (
                        <Link
                           to={"/surah/" + numbertosurah[dataDetails?.surat_sebelumnya?.nomor]}
                           className="flex gap-2 px-2 sm:py-2.5 sm:px-3 text-xs items-center font-semibold rounded-xl text-white bg-linear-to-r hover:bg-linear-to-t from-slate-900 to-slate-700 dark:from-slate-800 dark:to-slate-600 border-none focus:ring-2 ring-offset-2 ring-slate-900"
                        >
                           <span className="hidden sm:block">{dataDetails?.surat_sebelumnya?.nama_latin ?? "Memuat Data"}</span>
                           <svg
               ...
               <div className="border border-slate-200 pl-3 pr-1 rounded-lg bg-white dark:bg-gray-800/50 dark:border-gray-700 flex gap-2 items-center">
                  <div className="">
                     <p className="text-sm text-center font-medium">{dataDetails?.nama_latin ?? "Memuat Data"}</p>
                  ...
                  <button
                     onClick={togglePlay}
                     className="p-1 rounded-md bg-linear-to-r hover:bg-linear-to-t from-slate-800 to-slate-700 border-none hover:shadow-lg focus:ring-2 ring-offset-2 ring-slate-800"
                  >
                     {isPlaying ? (
               ...
               {(() => {
                  if (dataDetails?.surat_selanjutnya !== false) {
                     return (
                        <Link
                           to={"/surah/" + numbertosurah[dataDetails?.surat_selanjutnya?.nomor]}
                           className="flex gap-2 px-2 sm:py-2.5 sm:px-3 text-xs items-center font-semibold rounded-xl text-white bg-linear-to-r hover:bg-linear-to-t from-slate-900 to-slate-700 dark:from-slate-800 dark:to-slate-600 border-none focus:ring-2 ring-offset-2 ring-slate-900"
                        >
                           <span className="hidden sm:block">{dataDetails?.surat_selanjutnya?.nama_latin ?? "Memuat Data"}</span>
                           <svg
                              className="w-5 h-5 text-white block sm:hidden"
                              aria-hidden="true"
                              xmlns="http://www.w3.org/2000/svg"
