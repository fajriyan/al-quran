...
export default function Screensaver() {
   ...
   const prevVerse = () => setIndex((prev) => (prev - 1 + verses.length) % verses.length);
   ...
   useEffect(() => {
    ...
    return (
       <>
         <title>Quote Al Quran Digital | Baca Al Quran Mudah Tanpa Install</title>
         ...
         >
            ...
            {/* Controls */}
            {showControls && (
               <div data-html2canvas-ignore="true" className="absolute bottom-6 flex gap-3 flex-wrap justify-center transition-opacity">
                  <button onClick={prevVerse} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm backdrop-blur-sm transition">
                     Prev
                  </button>
                  <button onClick={nextVerse} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm backdrop-blur-sm transition">
                     Next
                  </button>
                  <button
                     ...
                     className={`px-4 py-2 rounded-xl text-sm backdrop-blur-sm transition relative overflow-hidden ${
                        isAutoplay ? "bg-white/10 hover:bg-white/20" : "bg-white/10 hover:bg-white/20"
                     }`}
                  >
                     {isAutoplay ? "Stop" : "Autoplay"}
                     {isAutoplay && <div className="absolute left-0 bottom-0 h-[3px] bg-white/70 transition-all" style={{ width: `${progress}%` }}></div>}
                  </button>
                  <button onClick={toggleFullscreen} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm backdrop-blur-sm transition">
                     {isFullscreen ? " Exit Fullscreen" : "⛶ Fullscreen"}
                  </button>
                  <button onClick={saveAsImage} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm transition">
                     Save as Image
                  </button>
                  <button onClick={() => setShowControls(false)} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm transition">
                     Hide Controls
                  </button>
               </div>
            )}
